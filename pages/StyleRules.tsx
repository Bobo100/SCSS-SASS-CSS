...
export default function StyleRules() {
    ...
    .child {
        color: blue;
                ...
                <p>
                    :active 狀態表示已被使用者點擊了且正在操作的鏈接，通常用來設定點擊時的樣式。<br />
                    :focus 狀態表示當前有焦點的鏈接，意味著當用戶在選擇該元素時，它會被高亮顯示或附帶其他樣式或行為。<br />
                    :visited 狀態表示訪問過的鏈接，也就是點擊過的鏈接。注意：由於安全問題，瀏覽器不允許訪問已訪問過的連接的某些屬性和樣式。<br />
                    :link 狀態表示正常的、未被訪問過的鏈接，通常用來設定該連接的初始樣式。
                </p>

                <p>
                    ::before 被用來在一個元素的內容前面加上新的內容，可以用 :before 的形式替代。<br />
                    ::after 被用來在一個元素的內容後面加上新的內容，可以用 :after 的形式替代。<br />
                    ::first-letter 被用來選擇第一行文本中的第一個字母或字符，可以用 :first-letter 的形式替代。<br />
                    ::first-line 被用來選擇元素內容的第一行，可以用 :first-line 的形式替代。
                </p>
